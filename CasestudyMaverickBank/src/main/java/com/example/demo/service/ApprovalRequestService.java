...
public class ApprovalRequestService {
	...
	private NewLoanListRepository newloanlistrepo;
	   ...
	   //customer
		 public String closeBankAccountsRequest(String accountNumber) {
			 ...
			 Account account= optionalaccount.get();
			   List<NewLoan> loans = newLoanrepo.findByAccountNumber(account.getAccountNumber());

			    boolean allLoansClosed = loans.stream()
			                                  .allMatch(loan -> loan.getStatus().equalsIgnoreCase("Closed"));
			    
			    if (!allLoansClosed) {
			        throw new IllegalArgumentException("Cannot close account as not all loans are closed for account number: " + accountNumber);
			    }
			 ApprovalRequest approvalRequest = new ApprovalRequest();
	...
	///
	 ...
	 public String approveLoanClose(int requestId) {
	  ...
	  public String approveLoan(int requestId,int loanId, double sanctionAmount, int period) {
